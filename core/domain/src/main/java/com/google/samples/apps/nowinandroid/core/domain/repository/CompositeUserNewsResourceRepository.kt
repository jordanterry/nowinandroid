...
class CompositeUserNewsResourceRepository @Inject constructor(
    ...
    override fun getUserNewsResources(): Flow<List<UserNewsResource>> =
        userNewsResources

    override fun getUserNewsResources(filterTopicIds: Set<String>): Flow<List<UserNewsResource>> =
        userNewsResources.map { it.filter { it.hasTopic(filterTopicIds) } }

    override fun getUserNewsResourcesForFollowedTopics(): Flow<List<UserNewsResource>> =
        userDataRepository.userData.flatMapLatest { getUserNewsResources(it.followedTopics) }
